...
import { MdPersonAdd, MdPerson, MdLock, MdVisibility, MdVisibilityOff, MdHourglassEmpty, MdVerifiedUser } from 'react-icons/md';
...
export default function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // call register then auto-login
      await authAPI.register(formData.username, formData.password, formData.adminCode || undefined);
      const loginRes = await authAPI.login(formData.username, formData.password);
      const { token, isAdmin } = loginRes.data;
      login({ username: formData.username, isAdmin }, token);
      navigate('/dashboard');
    } catch (err) {
        ...
        <div className="w-full max-w-[440px] glass-card rounded-xl p-8 shadow-2xl">
          <div className="flex flex-col items-center mb-8">
            <div className="size-16 rounded-full bg-primary/10 flex items-center justify-center mb-4 border border-primary/20">
              <MdPersonAdd className="text-primary text-3xl" />
            </div>
            <h1 className="text-slate-900 dark:text-white text-3xl font-bold leading-tight text-center">Create Account</h1>
            <p className="text-slate-500 dark:text-slate-400 mt-2 text-sm text-center">Join QuickTest today</p>
          </div>
            ...
            <div className="flex flex-col gap-2">
              ...
              <div className="relative group">
                ...
                <input
                  ...
                  onChange={handleChange}
                  placeholder="••••••••"
                  className={`w-full bg-slate-100/50 dark:bg-slate-800/50 border rounded-xl h-14 pl-12 pr-4 text-slate-900 dark:text-white placeholder:text-slate-500 focus:border-primary focus:ring-1 focus:ring-primary outline-none ${error.toLowerCase().includes('password') && !error.toLowerCase().includes('confirm') ? 'border-red-500' : 'border-slate-200 dark:border-slate-700'}`}
                  required
                />
              </div>
              <p className="text-xs text-slate-500 px-1">Minimum 6 characters</p>
            </div>

            <div className="flex flex-col gap-2">
              <label className="text-slate-700 dark:text-slate-300 text-sm font-medium px-1">Admin Code (optional)</label>
              <input
                type="text"
                name="adminCode"
                autoComplete="off"
                ...
                className="w-full bg-slate-100/50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-xl h-14 px-4 text-slate-900 dark:text-white placeholder:text-slate-500 focus:border-primary focus:ring-1 focus:ring-primary outline-none"
              />
            </div>

            <div className="flex flex-col gap-2">
              <label className="text-slate-700 dark:text-slate-300 text-sm font-medium px-1">Confirm Password</label>
              <div className="relative group">
                <MdLock className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-primary transition-colors" />
                <input
                  type={showPassword ? 'text' : 'password'}
                  ...
                  onChange={handleChange}
                  placeholder="••••••••"
                  className={`w-full bg-slate-100/50 dark:bg-slate-800/50 border rounded-xl h-14 pl-12 pr-12 text-slate-900 dark:text-white placeholder:text-slate-500 focus:border-primary focus:ring-1 focus:ring-primary outline-none ${error.toLowerCase().includes('confirm') ? 'border-red-500' : 'border-slate-200 dark:border-slate-700'}`}
                  required
                />
                <button
                  type="button"
